...
- (id)initWithPath:(NSString *)path withScaling:(eSCALING)_scaling withScreenSize:(CGSize)size
{
    ...
    image = [[NSImage alloc] initWithContentsOfFile:path];
		...
		InitTexture(&texture, NSSizeToCGSize(image.size), screenSize, scaling);
		...
		CGContextRef context = CGBitmapContextCreate(texture.data, texture.widthTexture, texture.heightTexture, 8, texture.widthTexture * texture.bytesPerPixel, CGImageGetColorSpace(imageRef), kCGImageAlphaNoneSkipLast);

		CGContextDrawImage(context, CGRectMake(0.0f, texture.heightTexture - texture.heightImage, texture.widthImage, texture.heightImage), imageRef);
